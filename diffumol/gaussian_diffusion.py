...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.alphas_cumprod_prev = np.append(1.0, self.alphas_cumprod[:-1]) # 1,a1,a1a2,a1a2a3...
        self.alphas_cumprod_next = np.append(self.alphas_cumprod[1:], 0.0)# a1a2,a1a2a3... 0.0
        assert self.alphas_cumprod_prev.shape == (self.num_timesteps,)

        # calculations for diffusion q(x_t | x_{t-1}) and others
        self.sqrt_alphas_cumprod = np.sqrt(self.alphas_cumprod) #开根号
        self.sqrt_one_minus_alphas_cumprod = np.sqrt(1.0 - self.alphas_cumprod) #1-根号下...
    ...
    def _token_discrete_loss(self, x_t, get_logits, input_ids, mask=None, truncate=False, t=None):
        ...
        decoder_nll = loss_fct(logits.contiguous().view(-1, logits.size(-1)), input_ids.contiguous().view(-1)).contiguous().view(input_ids.shape)
        if mask != None:
            decoder_nll *= mask
    ...
    def training_losses_seq2seq(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        """
        x_start_fix = x_start  # save the original x_0
        assert 'input_ids' in model_kwargs

        # Extract and prepare masks
        if self.num_props:
            input_ids_mask = model_kwargs.pop('input_mask').to(t.device)[:, self.num_props - 1:].contiguous()
        else:
        ...
        if self.num_props:
            ...
            x_start_mean = th.cat([props, model.model.module.get_embeds(input_ids_x[:, self.num_props:])], 1)
        else:
            x_start_mean = model.model.module.get_embeds(input_ids_x)

        # Compute variance and initial sample x_start
        ...
        # Add Gaussian noise if not provided
        if noise is None:
            noise = th.randn_like(x_start)

        # Diffusion step: sample x_t
        x_t = self.q_sample(x_start, t, noise=noise, mask=input_ids_mask)

        # Get logits function from model
        ...
        # Decoder NLL computation
        if self.num_props:
            decoder_nll = self._token_discrete_loss(
                x_start[:, 1:, :], get_logits, input_ids_x[:, self.num_props:].to(th.long)
            )
            terms["nll"] = self._token_discrete_loss(
                model_out_x_start[:, 1:, :], get_logits, input_ids_x[:, self.num_props:].to(th.long),
                mask=input_ids_mask[:, 1:], truncate=True, t=t
            )
        else:
            decoder_nll = self._token_discrete_loss(
            ...
            terms["nll"] = self._token_discrete_loss(
                model_out_x_start, get_logits, input_ids_x.to(th.long),
        ...
        if hasattr(actual_model, 'graph_gate') and actual_model.graph_gate.last_alpha is not None:
            # L2正则：防止门控值过大
            gate_values = actual_model.graph_gate.last_alpha
            gate_penalty = th.mean(gate_values ** 2)
            terms["gate_reg"] = 0.01 * gate_penalty
            
            # 监控：记录门控值统计（不参与梯度）
            with th.no_grad():
                terms["gate_mean"] = gate_values.mean()
                terms["gate_std"] = gate_values.std()
                terms["gate_max"] = gate_values.max()
            
            # 将正则化加入总损失
            terms["loss"] = terms["loss"] + terms["gate_reg"]
