...
class myTokenizer():
    ...
    def __init__(self, args):
        if args.vocab == 'bert':
            ...
            vocab_dict = {'[START]': 0, '[END]': 1, '[UNK]':2, '[PAD]':3,'[UNCONDITION]':4,'[PROP]':5}
    ...
    def encode_token(self, sentences,num_props=False,scaffold=False): 
        ...
        if scaffold and num_props:
            ...
            for seq in sentences:
                tmp=[]
                prop,scaf=[float(i) for i in seq[:num_props]],seq[num_props]
                tmp+=prop
...
def load_model_emb(args, tokenizer):
    ...
    if int(os.environ['LOCAL_RANK']) == 0:
        if os.path.exists(path_save):
            ...
            try:
                # Try to load the saved state dict - 添加 weights_only=True 解决 FutureWarning
                saved_state = torch.load(path_save, weights_only=True)
                saved_weight = saved_state['weight']
                current_vocab_size = tokenizer.vocab_size
                saved_vocab_size = saved_weight.size(0)
                
                if current_vocab_size != saved_vocab_size:
                    print(f'Vocab size mismatch: saved {saved_vocab_size}, current {current_vocab_size}')
                    print('Adjusting embedding size...')
                    
                    # Create new weight tensor with current vocab size
                    new_weight = torch.empty(current_vocab_size, args.hidden_dim)
                    torch.nn.init.normal_(new_weight)  # Initialize with normal distribution
                    
                    # Copy existing embeddings for tokens that still exist
                    min_size = min(current_vocab_size, saved_vocab_size)
                    new_weight[:min_size] = saved_weight[:min_size]
                    
                    # Update the model state dict
                    model.load_state_dict({'weight': new_weight})
                    
                    # Save the updated embeddings
                    torch.save(model.state_dict(), path_save)
                    print(f'Updated embedding saved with new vocab size: {current_vocab_size}')
                else:
                ...
                print('Reinitializing random embeddings...')
        ...
        # For non-rank0 processes, also handle potential size mismatches
        try:
            # 添加 weights_only=True 解决 FutureWarning
            saved_state = torch.load(path_save, weights_only=True)
            saved_weight = saved_state['weight']
            current_vocab_size = tokenizer.vocab_size
            saved_vocab_size = saved_weight.size(0)
            ...
            if current_vocab_size != saved_vocab_size:
                print(f'Vocab size mismatch in worker: saved {saved_vocab_size}, current {current_vocab_size}')
                # Create new weight tensor with current vocab size
                new_weight = torch.empty(current_vocab_size, args.hidden_dim)
            ...
            print(f'Error in worker loading embeddings: {e}')
            torch.nn.init.normal_(model.weight)
...
def load_defaults_config():
    ...
    """
    with open('diffumol/config.json', 'r') as f:
        return json.load(f)
